...
class Mdtht {
    ...
    addAnchorToHeadings() {
        ...
        allHeadings.forEach(heading => {
            ...
            // ID已经在setTagLevel中设置，这里只添加锚点图标
            
            // 创建锚点元素
            const anchor = document.createElement('a');
            anchor.className = 'heading-anchor';
            anchor.href = `#${heading.id}`;
            anchor.title = '点击跳转到此标题';
    ...
    setTagLevel() {
        ...
        for (let i = 0, len = allTag.length; i < len; i++) {
            // 获取标题文本并生成URL友好的格式
            const headingText = allTag[i].textContent.trim();
            
            // 特殊处理数字和点号的格式，如"3.1 swan证书激活"
            let slugifiedText = '';
            ...
            if (match) {
                ...
                // 余下的文本部分 - 不做额外处理，保留原始内容
                const textPart = match[3];
                
                // 完整组合ID - 确保点号已被替换为连字符且不会在后续处理中被移除
                slugifiedText = `${numberPart}-${textPart}`;
            } else {
                // 没有明显的数字编号部分，使用完整标题
                slugifiedText = headingText;
            }
            
            // 规范化ID：替换空格为连字符，移除特殊字符
            slugifiedText = slugifiedText
                .replace(/\s+/g, '-') // 将空格替换为连字符
                .replace(/[^\w\u4e00-\u9fa5\-]/g, '') // 只保留字母、数字、中文、连字符
                .toLowerCase() // 转为小写
                .replace(/-{2,}/g, '-') // 将多个连续连字符替换为单个连字符
                .replace(/^-|-$/g, ''); // 移除开头和结尾的连字符
            
            // 确保ID唯一性
            let uniqueId = slugifiedText;
            let counter = 1;
            while (usedIds.has(uniqueId)) {
                uniqueId = `${slugifiedText}-${counter}`;
                counter++;
            }
            
            usedIds.add(uniqueId);
            
            // 为了保持兼容性，保存原始的level-x格式信息作为data属性
            if (i === 0) {
                allTag[i].dataset.level = 'level-1';
            } else {
                /*后一个和从前一个比较*/
                //第2个标签数字 > 前1个的标签数字 ，则是前1个的子级
                if (this.getTagNumber(allTag[i]) > this.getTagNumber(allTag[i - 1])) {
                    allTag[i].dataset.level = allTag[i - 1].dataset.level + '.1';
                    continue;
                }
                //第2个标签数字 == 前1个的标签数字 ，则是前1个的同级
                if (this.getTagNumber(allTag[i]) == this.getTagNumber(allTag[i - 1])) {
                    allTag[i].dataset.level = this.getPrefixIndex(allTag[i - 1].dataset.level)[0] +
                        (this.getPrefixIndex(allTag[i - 1].dataset.level)[1] + 1);
                    continue;
                }
                ...
                if (this.getTagNumber(allTag[i]) < this.getTagNumber(allTag[i - 1])) {
                    for (let j = i - 1; j >= 0; j--) {
                        if (this.getTagNumber(allTag[i]) ==
                            ...
                            allTag[i].dataset.level = this.getPrefixIndex(allTag[j - 1].dataset.level)[0] + (this.getPrefixIndex(allTag[j - 1].dataset.level)[1] + 1);
                            break;
                            ...
                            allTag[i].dataset.level = this.getPrefixIndex(allTag[j].dataset.level)[0] +
                                (this.getPrefixIndex(allTag[j].dataset.level)[1] + 1);
                            //如果前一个：allTag[j] 是一级目录了,停止继续查找，结束循环
                            if (allTag[j].dataset.level.indexOf('.') === -1) {
                                break;
                            }
                        }
                    }
            ...
            // 设置唯一ID
            allTag[i].id = uniqueId;
        }
        
        return allTag.length > 0;
    }
     ...
     * 根据指 data-level 属性中 level- 中 "." 字符出现的次数判断
     * 0为一级，1为二级，依次类推
     * ----------------------------------------
    ...
    levelTagArr(level) {
        try {
            ...
            for (let i = 0, len = allTag.length; i < len; i++) {
                let number = this.findStrFre(allTag[i].dataset.level, '.');
                switch (number) {
                    case 0:
     ...
     * ----------------------------------------
     * @param str  当前目录的 data-level值 =>(level-1.1)
     */
    findCeilCatalog(str) {
        try {
            ...
            if (level > 1) {
                ...
                for (let i = 0, len = childArr.length; i < len; i++) {
                    //父目录
                    const pName = childArr[i].dataset.level;
    ...
    findAllCeilCatalog(str, ceilArr = []) {
        try {
            ...
            if (ceilCatalog.length > 0) { //不是1级目录
                let ceilName = ceilCatalog[0].dataset.level; //上一级目录class
        ...
        if (res) {
            ...
            if (level1.length > 0) { //有目录
                for (let i = 0, len = level1.length; i < len; i++) {
                    ...
                    liElement.innerHTML = `<a class="${level1[i].dataset.level}" href="#${level1[i].id}">
                <div>
                ...
                for (let j = firstLevelNum + 1; j <= 6; j++) {
                    ...
                    if (levelOther.length > 0) {
                        for (let k = 0, len = levelOther.length; k < len; k++) {
                            //当前目录id
                            let cName = levelOther[k].dataset.level;
                            let handleName = cName.slice(0, cName.lastIndexOf('.'));
                            //当前目录的上级目录
                            let ceilElement = this.findCeilCatalog(cName);
                            if (handleName == ceilElement[0].dataset.level) { //找到相应的上级目录
                                //上级目录li
                                let ceilLi = document.getElementsByClassName(
                                    ceilElement[0].dataset.level)[0].parentElement;
                                //子目录内容
                                let content = document.createElement('li');
                                content.innerHTML = `<a class="${cName}" href="#${levelOther[k].id}">
                        <div>
                ...
                for (let m = 0, len = level1.length; m < len; m++) {
                    let liElement = document.getElementsByClassName(
                        level1[m].dataset.level)[0].parentElement;
